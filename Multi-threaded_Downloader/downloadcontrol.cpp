...
//用url目标资源链接，通过header的ContentLengthHeader获得其size
...
//        qDebug() <<endl<<endl<< var<<endl<<endl<<var.toLongLong();
        reply->deleteLater();
        size = var.toLongLong();
        break;
    }
...
//写SETTING文件，用以续传
void DownloadControl::writeSetting()
{
    ...
    for(int i = 0;i < count;i ++)
    {
        ...
        //出bug，原：settings.setValue("READYSIZE" + index,threads.at(i)->readySize);
        if(threads.at(i)->readySize == 0)
        {
            settings.setValue("READYSIZE" + index,0);
        }
        else
        {
            settings.setValue("READYSIZE" + index,threads.at(i)->readySize - 1);
        }
    }
}
